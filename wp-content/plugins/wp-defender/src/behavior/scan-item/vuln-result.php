...
class Vuln_Result extends Behavior {
		...
		if ( isset( $data['name'], $data['version'], $data['bugs'] ) ) {
			...
			return [
				...
				/* translators: %s: Version of WP core, plugin or theme. */
					__( 'Vulnerability found in %s.', 'wpdef' ), $data['version']
				),
	...
	public function resolve() {
		...
		return new WP_Error(
			Error_Code::INVALID,
			__( 'Please try again! We could not find the issue type.', 'wpdef' )
		);
	}
	...
	private function upgrade_theme( $slug ) {
		...
		return new WP_Error(
			Error_Code::INVALID,
			__( "We couldn't update your theme. Please try updating with another method.", 'wpdef' )
		);
	}
	...
	private function remove_plugin( array $data ): array {
		...
		if ( $active ) {
			return [
				'type_notice' => 'error',
				'message' => __( 'This plugin cannot be removed because it is active.', 'wpdef' ),
			];
		}
		...
		if ( file_exists( $abs_path ) && ! $this->remove_vulnerability( $abs_path ) ) {
			return [
				'type_notice' => 'error',
				'message' => __( 'Defender does not have enough permission to remove this plugin.', 'wpdef' ),
			];
		}
	...
	private function remove_theme( array $data ): array {
		...
		if ( $active ) {
			return [
				'type_notice' => 'error',
				'message' => __( 'This theme cannot be removed because it is active.', 'wpdef' ),
			];
		}
		...
		if ( file_exists( $abs_path ) && ! $this->remove_vulnerability( $abs_path ) ) {
			return [
				'type_notice' => 'error',
				'message' => __( 'Defender does not have enough permission to remove this theme.', 'wpdef' ),
			];
		}
	...
	public function delete() {
		...
		if ( 'wp_core' === $data['type'] ) {
			return [
				'type_notice' => 'error',
				'message' => __( 'WordPress core cannot be removed.', 'wpdef' ),
			];
		} elseif ( 'plugin' === $data['type'] ) {
		...
		return new WP_Error(
			Error_Code::INVALID,
			__( "We couldn't remove this item.", 'wpdef' )
		);
	}
}
