...
Your temporary password is {{passcode}}. To finish logging in, copy and paste the temporary password into the Password field on the login screen.';
	...
	private function replace_scan_email_subject_issue_found( string $subject ): string {
		$subject = str_replace(
			__( 'Scan of {SITE_URL} complete.', 'wpdef' ),
			__( 'Malware Scan of {SITE_URL} is complete.', 'wpdef' ),
			$subject
		);

		return str_replace(
			__( '{ISSUES_COUNT} issues found.', 'wpdef' ),
			__( '{ISSUES_COUNT} issue(s) found.', 'wpdef' ),
			$subject
		);
	}
	...
	private function replace_scan_email_content_issue_found_new( string $content ): string {
		$content = str_replace(
			__( '{ISSUES_COUNT} vulnerabilities were identified for {SITE_URL} during a Malware Scan.', 'wpdef' ),
			__( 'Malware Scan identified {ISSUES_COUNT} issue(s) on {SITE_URL}.', 'wpdef' ),
			$content
		);

		return str_replace(
			__( 'See details for each issue below.', 'wpdef' ),
			__( 'The identified issue(s) is/are listed below.', 'wpdef' ),
			$content
		);
	}
	...
	private function upgrade_3_11_0(): void {
		...
		if ( ! empty( $option ) && is_string( $option ) ) {
			...
			if ( is_array( $old_settings ) && isset( $old_settings['global_ip_list'] ) ) {
				$model_global_ip = wd_di()->get( \WP_Defender\Model\Setting\Global_Ip_Lockout::class );
	...
	private function upgrade_3_12_0(): void {
		...
		set_site_transient( Config_Hub_Helper::CONFIGS_TRANSIENT_KEY, $configs, Config_Hub_Helper::CONFIGS_TRANSIENT_TIME );
