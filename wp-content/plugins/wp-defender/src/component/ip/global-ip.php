...
class Global_IP extends Component {
	...
	public function init(): void {
		...
		$this->global_list = is_array( $global_ip_list ) ? $global_ip_list : [];
		$this->allow_list = $global_ip_list['allow_list'] ?? [];
		$this->block_list = $global_ip_list['block_list'] ?? [];
	}
	...
	public function set_global_ip_list( array $data ) {
		if (
			...
			return new WP_Error( 'defender_hub_api_missing_params', __( 'Missing parameter(s)', 'wpdef' ) );
		}
		...
		if ( ! empty( $errors['allow_list'] ) || ! empty( $errors['block_list'] ) ) {
			return new WP_Error( 'defender_hub_api_invalid_ips', __( 'Invalid IP(s)', 'wpdef' ), $errors );
		} else {
			return $ret;
		}
	}
	...
	public function fetch_global_ip_list() {
		...
		if ( ! is_array( $data ) ) {
			...
			return new WP_Error( 'defender_hub_api_invalid_returned', __( 'API returned invalid data format.', 'wpdef' ) );
		}
	...
	public function add_to_global_ip_list( array $params ) {
		...
		if ( ! $is_allow_list && ! $is_block_list ) {
			return new WP_Error(
				...
				__( 'Invalid Global IP parameter(s) provided.', 'wpdef' )
			);
		}
