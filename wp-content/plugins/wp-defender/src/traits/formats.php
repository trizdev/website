...
trait Formats {
	...
	public function time_since( $since ): string {
		...
		$chunks = [
			[ 60 * 60 * 24 * 365, esc_html__( 'year', 'wpdef' ) ],
			[ 60 * 60 * 24 * 30, esc_html__( 'month', 'wpdef' ) ],
			[ 60 * 60 * 24 * 7, esc_html__( 'week', 'wpdef' ) ],
			[ 60 * 60 * 24, esc_html__( 'day', 'wpdef' ) ],
			[ 60 * 60, esc_html__( 'hour', 'wpdef' ) ],
			[ 60, esc_html__( 'minute', 'wpdef' ) ],
			[ 1, esc_html__( 'second', 'wpdef' ) ],
		];
	...
	public function get_times() {
		$times_interval = (array) apply_filters( 'defender_get_times_interval', [ '00', '30' ] );
		$data = [];
		for ( $i = 0; $i < 24; $i ++ ) {
	...
	/**
	 * We translate the datetime format from php to something that moment.js can understand.
	 *
	 * @param $format
	 *
	 * @return string
