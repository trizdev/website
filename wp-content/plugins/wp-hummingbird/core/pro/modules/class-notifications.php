...
class Notifications extends Module {
	...
	public function get_schedule_label_for( $module, $type ) {
		...
		// Reports.
		if ( 1 === $settings['frequency'] && ! empty( $settings['time'] ) ) {
			$daily_date_object = date_create( $settings['time'] );

			if ( $daily_date_object instanceof \DateTime ) {
				$label = sprintf( /* translators: %s - time */
					esc_html__( 'Daily, %s', 'wphb' ),
					date_format( $daily_date_object, 'h:00 A' )
				);
			}
		}

		// Weekly.
		if ( 7 === $settings['frequency'] && ! empty( $settings['day'] ) && ! empty( $settings['time'] ) ) {
			$weekly_date_object = date_create( $settings['day'] . ', ' . $settings['time'] );

			if ( $weekly_date_object instanceof \DateTime ) {
				$label = sprintf( /* translators: %s - week day, time */
					esc_html__( 'Weekly on %s', 'wphb' ),
					date_format( $weekly_date_object, 'l, h:00 A' )
				);
			}
		}

		// Monthly.
		if ( 30 === $settings['frequency'] && ! empty( $settings['day'] ) && ! empty( $settings['time'] ) ) {
			$monthly_date_object = date_create( $settings['time'] );

			if ( $monthly_date_object instanceof \DateTime ) {
				$label = sprintf( /* translators: %1$s - day of month, %2$s - time */
					esc_html__( 'Monthly/%1$s, %2$s', 'wphb' ),
					(int) $settings['day'],
					date_format( $monthly_date_object, 'h:00 A' )
				);
			}
