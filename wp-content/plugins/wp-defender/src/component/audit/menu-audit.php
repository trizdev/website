...
class Menu_Audit extends Audit_Event {
	...
	public function __construct() {
		add_action( 'admin_init', [ $this, 'cache_old_menu' ] );
	}
	...
	public function get_hooks(): array {
		return [
			'wp_create_nav_menu' => [
				'args' => [ 'term_id', 'menu_data' ],
				'callback' => [ self::class, 'menu_created_callback' ],
				'event_type' => Audit_Log::EVENT_TYPE_MENU,
				'action_type' => self::ACTION_CREATED,
			],
			'wp_update_nav_menu' => [
				'args' => [ 'menu_id', 'menu_data' ],
				'callback' => [ self::class, 'menu_updated_callback' ],
				'event_type' => Audit_Log::EVENT_TYPE_MENU,
				'action_type' => self::ACTION_UPDATED,
			],
			'wp_delete_nav_menu' => [
				'args' => [ 'term_id' ],
				'callback' => [ self::class, 'menu_deleted_callback' ],
				'event_type' => Audit_Log::EVENT_TYPE_MENU,
				'action_type' => self::ACTION_DELETED,
			],
			'wp_update_nav_menu_item' => [
				'args' => [ 'menu_id', 'menu_item_db_id', 'args' ],
				'callback' => [ self::class, 'menu_item_updated_callback' ],
				'event_type' => Audit_Log::EVENT_TYPE_MENU,
				'action_type' => self::ACTION_UPDATED,
			],
			'delete_post' => [
				'args' => [ 'post_ID' ],
				'callback' => [ self::class, 'menu_item_deleted_callback' ],
				'event_type' => Audit_Log::EVENT_TYPE_MENU,
				'action_type' => self::ACTION_DELETED,
			],
		];
	...
	public function menu_updated_callback() {
		...
		$menu_data = $func_args[1]['menu_data'];
		$post_array = filter_input_array( INPUT_POST );
		$new_items = isset( $post_array['menu-item-title'] ) ? array_keys( $post_array['menu-item-title'] ) : [];
		$old_menu = $this->get_cached_menu( $menu_id );
		$old_items = array_column( $old_menu['items'], 'item_id' );
	...
	public function get_cached_menu( $term_id ) {
		$cached = Array_Cache::get( 'menu_updated', 'audit', [] );
