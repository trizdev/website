...
class Forminator_Addon_Googlesheet_Quiz_Settings extends Forminator_Addon_Quiz_Settings_Abstract {
	...
	public function pick_name( $submitted_data ) {
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $name ) ) {
					throw new Forminator_Addon_Googlesheet_Exception( esc_html__( 'Please pick valid name' ) );
				}
	...
	public function setup_sheet( $submitted_data ) {
		...
		if ( $is_submit ) {
			...
			try {
				...
				$notification = array(
					'type' => 'success',
					'text' => '<strong>' . $this->addon->get_title() . '</strong> ' . esc_html__( 'Successfully created spreadsheet and connected to your quiz', 'forminator' ),
				);
				$is_close     = true;
