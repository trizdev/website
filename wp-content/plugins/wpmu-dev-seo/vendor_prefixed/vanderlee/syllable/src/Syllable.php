...
namespace Smartcrawl_Vendor\Vanderlee\Syllable;
...
use DOMText;
use DOMXPath;
use Smartcrawl_Vendor\Vanderlee\Syllable\Cache\Cache;
use Smartcrawl_Vendor\Vanderlee\Syllable\Cache\Json;
use Smartcrawl_Vendor\Vanderlee\Syllable\Hyphen\Hyphen;
use Smartcrawl_Vendor\Vanderlee\Syllable\Hyphen\Soft;
use Smartcrawl_Vendor\Vanderlee\Syllable\Hyphen\Text;
use Smartcrawl_Vendor\Vanderlee\Syllable\Source\File;
use Smartcrawl_Vendor\Vanderlee\Syllable\Source\Source;
/**
 * Main class.
 */
class Syllable
{
    ...
    public function hyphenateHtmlText($html)
    {
        ...
        $html = "<!DOCTYPE html PUBLIC '-//W3C//DTD HTML 4.0 Transitional//EN' 'http://www.w3.org/TR/REC-html40/loose.dtd'>" . '<html>' . '<head>' . "<meta http-equiv='content-type' content='text/html; charset={$charset}'>" . '</head>' . "<body>{$bodyContent}</body>" . '</html>';
        $dom = new DOMDocument();
        $dom->resolveExternals = \true;
        $dom->loadHTML($html, $this->libxmlOptions);
        // filter excludes
    ...
    private function parseHtmlText($html)
    {
        ...
        return [$bodyContent, $beforeBodyContent, $afterBodyContent];
    }
    /**
    ...
    private function parseWord($word)
    {
        ...
        if ($wordLength < $this->minHyphenLeft + $this->minHyphenRight || $wordLength < $this->minWordLength) {
            return [$word];
        }
    ...
    private function parseWordByPatterns($word, $wordLength = 0, $wordLowerCased = null)
    {
        ...
        $patternLength = $this->maxPattern < $textLength ? $this->maxPattern : $textLength;
        // Maximize
        $before = [];
