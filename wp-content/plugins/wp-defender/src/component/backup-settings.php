...
class Backup_Settings extends Component {
	...
	public function __construct() {
		$this->is_pro = ( new \WP_Defender\Behavior\WPMUDEV() )->is_pro();
	}
	...
	public function clear_keys(): void {
		$keys = get_site_option( self::INDEXER, [] );
		if ( empty( $keys ) ) {
	...
	public function clear_configs(): void {
		...
		delete_site_option( self::INDEXER );
		delete_site_option( self::KEY );
		delete_site_transient( \WP_Defender\Component\Config\Config_Hub_Helper::CONFIGS_TRANSIENT_KEY );
	}
	...
	public function index_key( string $key ): void {
		...
		if ( is_array( $keys ) ) {
			...
			// The first config.
			update_site_option( self::INDEXER, [ $key => $key ] );
		}
	}
	...
	public function restore_data( array $data, string $request_reason = 'plugin' ) {
		...
		foreach ( $data as $module => $module_data ) {
			...
			if ( is_object( $controller ) || is_array( $controller ) ) {
				 ...
				 * Scan, Firewall, Audit, Blocklist Monitor, Pwned Passwords,
			...
			case 'iplockout':
				if ( $all_firewall_lockouts ) {
					return [
						...
						new \WP_Defender\Controller\Global_Ip(),
	...
	public function import_module_strings( array $data ): array {
		...
		foreach ( $data['configs'] as $key => $config ) {
			if (
				...
				$data['strings']['scan'] = wd_di()->get( Controller_Scan::class )->config_strings( $config, $this->is_pro );
			} elseif ( 'iplockout' === $key ) {
				$data['strings']['iplockout'] = wd_di()->get( Controller_Firewall::class )->config_strings( $config, $this->is_pro );
			} elseif ( 'audit' === $key ) {
				// Additional check for Free version.
				$config = is_array( $config ) ? $config : [];
				$data['strings']['audit'] = wd_di()->get( Audit_Logging::class )->config_strings( $config, $this->is_pro );
			} elseif ( 'blocklist_monitor' === $key ) {
				// Additional check for Free version.
				$config = is_array( $config ) ? $config : [];
				$data['strings']['blocklist_monitor'] = $this->format_blocklist_monitor_strings( $config, $this->is_pro );
			} elseif ( 'pwned_passwords' === $key ) {
				$data['strings']['pwned_passwords'] = wd_di()->get( Controller_Password_Protection::class )->config_strings( $config, $this->is_pro );
			}
		}
