...
class Forminator_Addon_Slack_Form_Hooks extends Forminator_Addon_Form_Hooks_Abstract {
	...
	public function __construct( Forminator_Addon_Abstract $addon, $form_id ) {
		...
		$this->_submit_form_error_message = esc_html__( 'Slack failed to process submitted data. Please check your form and try again', 'forminator' );
	}
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => esc_html__( 'Slack Integration', 'forminator' ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => esc_html__( 'Sent To Slack', 'forminator' ),
				'value' => $is_sent,
			);
		}
		...
		if ( Forminator_Addon_Slack::is_show_full_log() ) {
			...
			if ( isset( $status['data_sent'] ) ) {
				$sub_entries[] = array(
					'label' => esc_html__( 'Data sent to Slack', 'forminator' ),
			...
			if ( isset( $status['data_received'] ) ) {
				$sub_entries[] = array(
					'label' => esc_html__( 'Data received from Slack', 'forminator' ),
