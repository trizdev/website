...
class Notification extends Event {
	...
	public function show_actions_with_subscription() {
		...
		if ( 'subscribed' === $context ) {
			...
			/* translators: 1. Module title. 2. Unsubscribed link. */
				__( 'You are now subscribed to receive %1$s. Made a mistake? <a href="%2$s">Unsubscribe</a>', 'wpdef' ),
			...
			/* translators: %s: Module title. */
				__( 'You are now unsubscribed from %s.', 'wpdef' ),
				'<strong>' . $m->title . '</strong>'
			);
		} else {
			return null;
	...
	public function validate_email( Request $request ): Response {
		...
		if ( filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
			...
			return new Response( false, [ 'error' => __( 'Invalid email address.', 'wpdef' ) ] );
		}
	}
	...
	public function resend_invite_email( Request $request ): Response {
		...
		if ( ! is_object( $model ) ) {
			return new Response( false, [ 'message' => __( 'Module not found.', 'wpdef' ) ] );
		}
		...
		if ( $sent ) {
			return new Response( true, [ 'message' => __( 'Invitation sent successfully.', 'wpdef' ) ] );
		}
