	...
	private function send_to_user( string $email, string $name, Lockout_Log $model, string $template, object $service ): void {
		...
		if ( 'login-lockout' === $template ) {
			...
			if ( 'permanent' === $settings->lockout_type ) {
				...
				$string = sprintf(
					__( 'They have been locked out for <strong>%1$s %2$s</strong>.', 'wpdef' ),
					esc_html( $settings->duration ),
					esc_html( $settings->duration_unit )
				);
			}
			$text = sprintf(
				$text,
				...
				$network_site_url,
				$network_site_url,
				esc_html( $settings->attempt ),
				esc_html( $model->tried ),
				$string
			);
		}
