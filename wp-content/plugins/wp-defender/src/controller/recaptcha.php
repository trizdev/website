...
class Recaptcha extends Event {
	...
	protected function add_actions() {
		...
		if ( $display_for_known_users && in_array( Recaptcha_Component::DEFAULT_COMMENT_FORM, $locations, true ) ) {
			...
			add_filter( 'comment_form_defaults', [ $this, 'comment_form_defaults' ], 10 );
	...
	protected function get_api_url(): string {
		if ( isset( $this->recaptcha_type ) ) {
			...
			} elseif ( in_array( $this->recaptcha_type, [ 'v2_checkbox', 'v2_invisible' ], true ) ) {
				return esc_url(
					sprintf( 'https://www.google.com/recaptcha/api.js?hl=%s&render=explicit', $this->language )
				);
			}
		}
	...
	protected function display_recaptcha(): string {
		...
		$id = \WP_Defender\Component\Crypt::random_int( 0, mt_getrandmax() );
	...
	public function load_recaptcha_preview( Request $request ): Response {
		...
		if ( isset( $this->recaptcha_type ) && ! empty( $this->public_key ) ) {
			$html .= '<script src="https://www.google.com/recaptcha/api.js?hl=' . esc_attr( $language ) . '&render=explicit&onload=' . $onload . '" async defer></script>' . $js;
