...
class Security_Tweaks extends Event {
	...
	public function revert( Request $request ) {
		...
		$slug = $data['slug'] ?? false;
		$tweak = $this->get_tweak( $slug );
		$invalid = [ 'message' => __( 'Invalid request', 'wpdef' ) ];
		if ( ! is_object( $tweak ) ) {
			return new Response(
				false,
				$invalid
	...
	public function recheck( Request $request ): Response {
		...
		if ( ! is_object( $tweak ) ) {
			return new Response(
				false,
				[ 'message' => __( 'The status cannot be verified as the request contains an invalid slug.', 'wpdef' ) ]
			);
		}
		...
		if ( true === $ret ) {
			$this->ajax_response( __( 'Security recommendation successfully resolved.', 'wpdef' ), true, 1 );
		}
		...
		/* translators: %s: Tweak title. */
			__( '%s is not actioned. Please ensure that all the instructions are followed.', 'wpdef' ),
