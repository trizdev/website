...
class Data_Tracking extends Event {
	...
	public function get_tracking_modal(): array {
		$title = __( 'Help Us Enhance Your Site\'s Security', 'wpdef' );

		$current_user = wp_get_current_user();

		$desc = sprintf(
			/* translators: %s: user display name */
			__( 'Hey there! %s, Defender is dedicated to protecting your WordPress website from hackers and malware. However, our mission is more effective with your collaboration. By opting in to share anonymous usage data, you help us refine and enhance our plugin for everyone\'s benefit.', 'wpdef' ),
			'<strong>' . esc_html( $current_user->display_name ) . '</strong>'
		);
		$desc .= '<br/><br/>';
		$desc .= sprintf(
			/* translators: %s: Link. */
			__( 'Your privacy is important to us. We guarantee that your data stays anonymous and your identity stays secure. Learn more about our usage tracking <a href="%s" target="_blank">here<a>.', 'wpdef' ),
		...
		return [
			...
			'banner_alt' => __( 'Help us improve Defender', 'wpdef' ),
			'optin_button_title' => __( 'OPT IN', 'wpdef' ),
			'skip_button_title' => __( 'Skip for now', 'wpdef' ),
