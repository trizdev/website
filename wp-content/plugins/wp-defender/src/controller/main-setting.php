...
class Main_Setting extends Event {
		...
		if ( update_site_option( $key, $data ) ) {
			...
			return new Response(
				...
				[
					'message' => sprintf(
						/* translators: %s: Config name. */
						__( '%s config saved successfully.', 'wpdef' ),
						'<strong>' . $name . '</strong>'
					),
					'configs' => Config_Hub_Helper::get_fresh_frontend_configs( $this->service ),
				]
	...
	public function apply_config( Request $request ) {
		...
		if ( $restore_result ) {
			...
			$message .= '<br/>';
			$message .= __( 'This will auto reload now.', 'wpdef' );
	...
	public function delete_config( Request $request ) {
		...
		if ( isset( $config['is_removable'] ) && ! $config['is_removable'] ) {
			return new Response(
				false,
				[
					'message' => __( 'Config can\'t be removed', 'wpdef' ),
				]
			);
		}
	...
	private function apply_config_recommendations_error_message(): Response {
		$message = sprintf(
		/* translators: %s: Link. */
			__( 'There was an issue with applying some of the tweaks from the <strong>Recommendations</strong> tab because we cannot make changes to your <strong>wp-config.php</strong> file. Please see our <a target="_blank" href="%s">documentation</a> to apply the changes manually.', 'wpdef' ),
