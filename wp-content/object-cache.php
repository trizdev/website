 ...
 *
 *
 * Adapted from https://github.com/humanmade/wordpress-pecl-memcached-object-cache. Uses newer more powerful phpX-memcached extension.
 * Implements changes pioneered by Human Made to how autoloading options works in WordPress. Concurrency problems in WordPress
 * usually boil down to https://core.trac.wordpress.org/ticket/31245 (commonly known as the “alloptions” bug) in some shape or form.
 * Solves race condition issues with overly-large alloptions cache keys by separating them into their own keys and using *_multi()
 * specialty functions in Memcached php extension to fetch them all at once still.
 */

// Changes salt in staging environments to avoid key conflicts
if ( isset( $_SERVER['WPMUDEV_HOSTING_ENV'] ) ) {
	define( 'WP_CACHE_KEY_SALT', $_SERVER['WPMUDEV_HOSTING_ENV'] );
} elseif ( defined( 'WP_CLI' ) && WP_CLI ) {
	if ( '/var/web/site/public_html' === $_SERVER['DOCUMENT_ROOT'] ) {
		$environment = 'production';
	} elseif ( '/var/web/staging/public_html' === $_SERVER['DOCUMENT_ROOT'] ) {
		$environment = 'staging';
	} else {
		$environment = 'production';
	}
	define( 'WP_CACHE_KEY_SALT', $environment );
}

// Always assume that Object Caching is enabled if there's no constant.
if ( ! defined( 'WPMUDEV_HOSTING_OBJECT_CACHE_ENABLED' ) ) {
	define( 'WPMUDEV_HOSTING_OBJECT_CACHE_ENABLED', true );
}

if ( WPMUDEV_HOSTING_OBJECT_CACHE_ENABLED && get_cfg_var('wpmudev.hosting_plan') != 'quantum' ) {
	/**
	...
	function wp_cache_flush( $delay = 0 ) {
		...
		/*if ( ! defined( 'WP_CLI' ) || ! WP_CLI ) {
			trigger_error( sprintf( 'wp_cache_flush() is only allowed via WP CLI. Called in %s line %d', $caller['file'], $caller['line'] ), E_USER_NOTICE );
			return false;
		}*/
		trigger_error( sprintf( 'wp_cache_flush() used, this is broadly not recommended. Called in %s line %d', $caller['file'], $caller['line'] ), E_USER_NOTICE );
		global $wp_object_cache;
