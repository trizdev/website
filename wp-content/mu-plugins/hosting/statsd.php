...
Copyright 2019 UglyRobot Web Development (http://uglyrobot.com)
...
if ( ! defined( 'STATSD_SAMPLE_RATE' ) ) {
	define( 'STATSD_SAMPLE_RATE', 1 );
} //allows you to override the sample rate for calls that run every page load. Example 0.5 would be roughly 50% of pageloads.

define( 'STATSD_NAMESPACE', '' ); // we don't need a namespace in our hosted environment.
...
class WordPress_StatsD {
	...
	public function __construct() {
		...
		add_action( 'rest_api_init', array( &$this, 'rest_api_call' ) );
	...
	public function pre_http( $false, $r, $url ) {
		$this->statsd->startTiming( "wordpress.http.requests" );
	...
	public function post_http( $response, $type, $class, $args, $url ) {
		...
		$this->statsd->endTiming( "wordpress.http.requests" );
		$this->statsd->increment( "wordpress.http.counts" );
		$this->statsd->endBatch();
	}

	public function num_queries() {
		$this->statsd->count( "wordpress.wpdb.queries", get_num_queries(), STATSD_SAMPLE_RATE );
	}

	public function load_time() {
		global $timestart;
		// can't use timer_stop() because it's i18n formatted.
		$timeend   = microtime( true );
		$timetotal = $timeend - $timestart;
		$load_time = round( 1000 * $timetotal );
		$this->statsd->timing( "wordpress.load_time", $load_time, STATSD_SAMPLE_RATE );
	}

	public function xmlrpc_call( $type ) {
		$this->statsd->increment( "wordpress.xmlrpc" );
	}

	public function rest_api_call( $wp_rest_server ) {
		$this->statsd->increment( "wordpress.rest_api" );
	}
