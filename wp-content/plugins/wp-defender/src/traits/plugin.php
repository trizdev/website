...
trait Plugin {
	...
	public function check_plugin_on_wp_org( $slug ): array {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			$message = __( 'Plugin unknown error.', 'wpdef' );
		...
		if ( ! is_array( $results ) ) {
			return [
				'message' => __( 'Plugin response is not in expected format.', 'wpdef' ),
				'success' => false,
			];
		}

		return [
			'message' => __( 'Plugin exists in WordPress respository.', 'wpdef' ),
			'success' => true,
		];
	}
