...
require_once dirname( __FILE__ ) . '/class-forminator-addon-mailjet-form-settings-exception.php';

/**
 * Class Forminator_Addon_Mailjet_Form_Settings
 * Form Settings Mailjet Process
 */
class Forminator_Addon_Mailjet_Form_Settings extends Forminator_Addon_Form_Settings_Abstract {
	...
	private function save_settings( $submitted_data, $list_name ) {
		...
		$this->addon_form_settings['mail_list_name'] = $list_name;
		...
		if ( ! $api_error ) {
			...
			if ( $this->addon->is_form_connected( $this->form_id ) ) {
				$buttons['disconnect']['markup'] = Forminator_Addon_Abstract::get_button_markup(
					...
					esc_html__( 'Deactivate Mailjet from this Form.', 'forminator' )
				);
			}
	...
	public function map_fields( $submitted_data ) {
		...
		try {
			...
			foreach ( $fields_list_ids as $key ) {
				$fields_map[ $key ] = $submitted_data['fields_map'][ $key ] ?? $this->addon_form_settings['fields_map'][ $key ] ?? '';
			}
		...
		$html .= '<p class="sui-description">' . esc_html__( 'Lastly, match up your form fields with the campaign fields to ensure that the data is sent to the right place.', 'forminator' ) . '</p>';
		$html .= $error_message;
		$html .= '</div>';
		$html .= '<form enctype="multipart/form-data">';
	 ...
	 *
	 * @param array $mailjet_fields_list List of Mailjet fields.
	...
	public function step_map_fields_validate( $mailjet_fields_list, $post_data ) {
		...
		if ( ! isset( $post_data['fields_map'] ) ) {
			...
			throw new Forminator_Addon_Mailjet_Exception( $this->_update_form_settings_error_message );
		}
		...
		foreach ( $post_data as $mailjet_field_tag => $forminator_field_id ) {
			$this->addon_form_settings['fields_map'][ $mailjet_field_tag ] = $post_data[ $mailjet_field_tag ];
		}

		$input_exceptions = new Forminator_Addon_Mailjet_Form_Settings_Exception();
		// email is required.
		if ( empty( $post_data['email'] ) ) {
			$this->_update_form_settings_error_message = esc_html__( 'Please choose valid Forminator field for email address.', 'forminator' );
		...
		foreach ( $this->addon_form_settings['fields_map'] as $mailjet_field_tag => $forminator_field_id ) {
			...
			if ( is_array( $forminator_field_id ) ) {
				...
				foreach ( $forminator_field_id as $addr => $field_id ) {
					...
					if ( ! empty( $address_value ) && ! in_array( $field_id, $forminator_field_element_ids, true ) ) {
						...
						$input_exceptions->add_sub_input_exception( $this->_update_form_settings_error_message, $mailjet_field_tag, $addr );
					}
				}
			}
			if ( ! is_array( $forminator_field_id ) && ! in_array( $forminator_field_id, $forminator_field_element_ids, true ) ) {
				...
				$this->_update_form_settings_error_message =
					/* translators: %s: Mailjet Field name */
					sprintf( esc_html__( 'Please choose valid Forminator field for %s.', 'forminator' ), esc_html( $mailjet_field_name ) );
				$input_exceptions->add_input_exception( $this->_update_form_settings_error_message, $mailjet_field_tag );
			}
		}
